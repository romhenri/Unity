...
{
	[CustomEditor(typeof(RuleTileCustom))]
	[CanEditMultipleObjects]
	internal class RuleTileCustomEditor : Editor
	{
		...
		public RuleTileCustom tile { get { return (target as RuleTileCustom); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
				tile.m_TilingRules = new List<RuleTileCustom.TilingRule>();

			m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(RuleTileCustom.TilingRule), true, true, true, true);
		...
		{
			RuleTileCustom.TilingRule rule = tile.m_TilingRules[index];
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTileCustom.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case RuleTileCustom.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case RuleTileCustom.TilingRule.Neighbor.NotThis:
								GUI.DrawTexture(r, arrows[9]);
								break;
						}
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_Neighbors[index] = (RuleTileCustom.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + 1) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case RuleTileCustom.TilingRule.AutoTransform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case RuleTileCustom.TilingRule.AutoTransform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case RuleTileCustom.TilingRule.AutoTransform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_AutoTransform = (RuleTileCustom.TilingRule.AutoTransform)(((int)tilingRule.m_AutoTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public RuleTileCustom.TilingRule m_Rule;
			public RuleTileCustom.TilingRule.AutoTransform m_NewValue;

			public MenuItemData(RuleTileCustom.TilingRule mRule, RuleTileCustom.TilingRule.AutoTransform mNewValue)
			{
				this.m_Rule = mRule;
		...
		private void SpriteOnGUI(Rect rect, RuleTileCustom.TilingRule tilingRule)
		{
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTileCustom.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_AutoTransform = (RuleTileCustom.TilingRule.AutoTransform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_AutoTransform);
			...
			tilingRule.m_Output = (RuleTileCustom.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == RuleTileCustom.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == RuleTileCustom.TilingRule.OutputSprite.Random)
			{
			...
			if (tilingRule.m_Output != RuleTileCustom.TilingRule.OutputSprite.Single)
			{
